...
class DEARoITransformer(BaseDetectorNew, RPNTestMixin):
    def __init__(self,
                 backbone,
                 neck=None,
                 shared_head=None,
        ...
        super(DEARoITransformer, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(DEARoITransformer, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_fcos:
            ...
            for i in range(len(fcos_bboxes_pred)):
                fcos_bboxes_reg=fcos_bboxes_pred[i][0]
                if len(fcos_bboxes_reg.shape) >= 2:
                    fcos_proposal[i] = fcos_bboxes_reg
                else:
                    fcos_proposal[i] = fcos_bboxes_reg.unsqueeze(0)
        ...
        if self.with_bbox or self.with_mask:
            ...
            for i in range(num_imgs):
                assign_result = bbox_assigner.assign(
                    proposal_list1[i], gt_bboxes[i], gt_bboxes_ignore[i],
                    gt_labels[i])
